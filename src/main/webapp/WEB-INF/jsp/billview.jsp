...
<div class="right">
     ...
     <div class="providerView">
         <p><strong>商品编号：</strong><span>${bill.billCode }</span></p>
         ...
         <p><strong>成本金额：</strong><span>${bill.totalPrice }</span></p>
         <p><strong>出售金额：</strong><span>${bill.outPrice }</span></p>
         <p><strong>库存：</strong><span>${bill.stock }</span></p>
         <p><strong>供应商：</strong><span>${bill.providerName }</span></p>
         <p><strong>是否付款：</strong>
         	<span>
         		<c:if test="${bill.isPayment == 1}">未上架</c:if>
				<c:if test="${bill.isPayment == 2}">已上架</c:if>
			</span>
		</p>
